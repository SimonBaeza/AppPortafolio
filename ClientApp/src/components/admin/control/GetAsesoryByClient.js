...
export class GetAsesoryByClient extends Component {
  constructor(props) {
    super(props);
    this.state = {
      formValues: {
  ...
  render() {
    const isAsesoryFound = this.state.data.length > 0;
    ...
    return (
      <Container>
        <Row>
          <Col lg="12">
            <Jumbotron>
              <h1> Buscar Asesorías por Cliente </h1>
              <hr />
              <div>
                <div1>
                ...
                {isFirstChange && !isLoading ? null : !isFirstChange &&
                  isLoading ? (
                  Spinner
                ) : !isFirstChange && !isLoading && isAsesoryFound ? (
                  <React.Fragment>
                    <div2>
                      <Table>
                        <thead>
